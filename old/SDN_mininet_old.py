...
def STREAM(STREAM_SRC):
	...
	info('*** Preparing Stream Output Directories . . .\n');
	...
	for i in range(DESTINATION_COUNT):
		dest_host_list[i].cmd(	'cd \'' + TARGET_DIR + '\' && '
								'java -jar \'recordVLC.jar\' \'' + INT2IP(STREAM_IP) + '\' \'' + str(STREAM_PORT) + '\' \'' + REC_DIR + os.path.sep + 'rec_' + str(i) + '.ts\' '
								'&> \'' + LOGS_DIR + os.path.sep + 'recordVLC_' + str(i) + '.log\' 2>&1 &');
	...
	source_host.cmd('cd \'' + TARGET_DIR + '\' && '
					'java -jar \'streamVLC.jar\' \'' + INT2IP(STREAM_IP) + '\' \'' + str(STREAM_PORT) + '\' \'' + STREAM_DESTDIR + os.path.sep + 'mod_' + SOURCE_FILENAME + '\' '
					'> \'' + LOGS_DIR + os.path.sep + 'streamVLC.log\' 2>&1');
	
	# Calculate PSNR for each Recording
	info('*** Calculating PSNR . . .\n');
	for i in range(DESTINATION_COUNT):
		dest_host_list[i].cmd(	'ffmpeg -i \'' + REC_DIR + os.path.sep + 'rec_' + str(i) + '.ts\' '
	...
	info('*** Streaming Successfully Completed!\n');
...
class CustomCLI(CLI):
	def do_stream(self, _line):
		...
		if len(argv) > 0 and os.path.isfile(argv[0]) is True:
			...
			info('*** Invalid Path!\n');
			return;
		print(str(argv) + '\t' + str(arg));
		print(str(len(argv)) + '\t' + str(len(arg)));
